# PowerShell script to test the Rust SQL Server shared library on Linux

# Configuration
$SoPath = "./target/release/libsqlthinkrs.so"
...
if (-not (Test-Path $SoPath)) {
    Write-Host "ERROR: Shared library not found at $SoPath" -ForegroundColor Red
    Write-Host "Please run: ./build.sh" -ForegroundColor Yellow
    exit 1
}

# Resolve full path so DllImport can locate the native library
$SoFullPath = (Resolve-Path $SoPath).Path
...
public class SqlThinkRS {
    [DllImport("$SoFullPath", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
    public static extern IntPtr ConnectDb(string connectionString);

    [DllImport("$SoFullPath", CallingConvention = CallingConvention.Cdecl)]
